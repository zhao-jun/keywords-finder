...
const search = async (searchPath, loop) => {
  ...
  for (let file of files) {
    if (fs.statSync(path.join(searchPath, file)).isFile()) {
          ...
          if (stat.size < 12000) {
            ...
            if (text) {
              keyWords.map(keyWord => {
                if (text.includes(keyWord)) results.push(` ${path.join(searchPath, file)} 中含有 ${keyWord} 请修改`)
              })
            }
          }
        } catch (error) {
      ...
      if (/.docx$/.test(file)) {
        let result = await mammoth.extractRawText({ path: path.join(searchPath, file) })
        keyWords.map(keyWord => {
          if (result.value.includes(keyWord)) results.push(` ${path.join(searchPath, file)} 中含有 ${keyWord} 请修改`)
        })
      }
    } else {
...
const init = () => {
  // 搜索结果、error存在则删除
  if (fsExistsSync(path.join(searchPathTop, resName))) fs.unlinkSync(path.join(searchPathTop, resName))
  if (fsExistsSync(path.join(searchPathTop, resError))) fs.unlinkSync(path.join(searchPathTop, resError))
  const data = fs.readFileSync(resolve('keywords.txt'), 'UTF-8')
